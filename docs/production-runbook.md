...
排泄メモと点眼画面の自動スクロールを追加しました。本番への適用・公開は未実施です。

1. 同期済みの食事JSONと介護JSONを保存します。
2. 第13節の`202609060001_edit_health_event_time.sql`が未適用なら先に適用します。続けて[`202609100001_health_event_notes.sql`](../supabase/migrations/202609100001_health_event_notes.sql)をSQL Editorで実行します。既存データを消去する必要はありません。
3. ローカルテスト・レビュー後にcommit・pushし、Pagesの公開完了を確認します。全端末を再起動してキャッシュv8へ更新します。
4. 小／大→任意メモ→記録する、空欄での保存、履歴のメモ要約と全文展開、時刻・メモの編集、取消しを確認します。2端末の同時編集では後の保存が拒否され、入力が残ることも確認します。
5. 介護JSONにメモと変更履歴が含まれることを確認します。過去のメモなし記録も閲覧・編集できます。
6. iPhoneで「点眼」を押し、現在時刻に最も近い回へ移動することを確認します。通知からは該当する今日の回を優先し、過去の通知など対象がない場合は今日の直近へ移動します。手動スクロール後はタイマー更新や同期で位置を戻さないことを確認します。

VAPID・Cron・Edge Functionの変更はありません。新RPC適用前は排泄の新規記録・編集が失敗するため、DB migrationを公開より先に行ってください。

### iPhone通知の相談（延期・未実装）

2026-09-10のユーザー判断により、通知改善・再通知・アラーム対応は一旦延期します。再開の依頼までは追加開発を行わず、既存のPush通知を継続します。以下は将来の検討用メモです。

利用端末はiPhone。通知の文面改善、未実施の場合だけの間隔付き再通知、利用者が音を有効化した上での前面表示中の音などは追加候補です。再通知には完了・取消し・通知OFF・担当引き継ぎ時の停止条件も必要です。

PWAのWeb PushはOSの通知設定・集中モードに従い、閉じた状態で鳴り続ける目覚まし時計の代わりにはできません。今すぐ使うなら標準「時計」のアラーム／タイマーを併用する方法があります。定時のアラームは消音・おやすみモードでも鳴ります（[Appleサポート](https://support.apple.com/ja-jp/118444)）。アプリ連動のアラームは、iOS 26以降のネイティブアプリでAlarmKitと利用者の許可を使う別開発が候補です。

参考: [WebKitのiOS Web Push説明](https://webkit.org/blog/13878/web-push-for-web-apps-on-ios-and-ipados/)、[AppleのAlarmKit説明](https://developer.apple.com/videos/play/wwdc2025/230/)。
