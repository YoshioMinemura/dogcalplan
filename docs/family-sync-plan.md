...
排泄は小／大の後に任意メモを入力して保存する方式となり、履歴から時刻・メモを編集できる。追加migration `202609100001_health_event_notes.sql`を日付順に適用し、Pagesを公開して全端末をキャッシュv8へ更新する。本番適用・公開は未実施。2端末でメモの共有・同時編集拒否・取消し履歴と、iPhoneの通知から対象点眼への移動を確認する。運用開始手順書第14節を参照。
